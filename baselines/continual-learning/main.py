...
task_params.add_argument('--experiment', type=str, default='cifar10-resnet',
                         choices=['permMNIST', 'splitMNIST', 'cifar10', 'cifar100', 'omniglot10',
                                  'cifar10-resnet', 'cifar100-resnet'])
task_params.add_argument('--scenario', type=str, default='class', choices=['task', 'domain', 'class'])
task_params.add_argument('--tasks', default=5, type=int, help='number of tasks')
...
train_params.add_argument('--batch', type=int, default=128, help="batch-size") # original: 128
...
eval_params.add_argument('--metrics', type=bool, default=True, help="calculate additional metrics (e.g., BWT, forgetting)")
...
def run(args, verbose=False):
    ...
    if args.metrics:
        ...
        if args.scenario in ('task', 'domain'):
            ...
            R = R.reindex(['at start'] + ['after task {}'.format(i + 1) for i in range(args.tasks)])
            ACCs = [(R.loc['after task {}'.format(args.tasks), 'task {}'.format(i + 1)]) for i in range(args.tasks)]
            BWTs = [(R.loc['after task {}'.format(args.tasks), 'task {}'.format(i + 1)] - \
                     R.loc['after task {}'.format(i + 1), 'task {}'.format(i + 1)]) for i in range(args.tasks-1)]
            FWTs = [0. if args.use_exemplars else (
                R.loc['after task {}'.format(i+1), 'task {}'.format(i+2)] - R.loc['at start', 'task {}'.format(i+2)]
