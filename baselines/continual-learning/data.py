...
class SubDataset(Dataset):
    ...
    def __init__(self, original_dataset, sub_labels, target_transform=None, has_resnet=False):
        super().__init__()
        self.dataset = original_dataset
        self.sub_indeces = []
        self.has_resnet = has_resnet
        if self.has_resnet:
    ...
    def __getitem__(self, index):
        ...
        if self.has_resnet:
            with torch.no_grad():
                sample = (self.resnet(sample[0].unsqueeze(0)), sample[1])
...
DATASET_CONFIGS = {
    'mnist':           {'size': 32 ** 2, 'channels': 1, 'classes': 10},
    'mnist28':         {'size': 28 ** 2, 'channels': 1, 'classes': 10},
    'cifar10':         {'size': 224 ** 2, 'channels': 3, 'classes': 10},
    'cifar100':        {'size': 224 ** 2, 'channels': 3, 'classes': 100},
    'cifar10-resnet':  {'size': 512, 'channels': 1, 'classes': 10},
    'cifar100-resnet': {'size': 512, 'channels': 1, 'classes': 100},
    'omniglot10':      {'size': 28 ** 2, 'channels': 1, 'classes': 500}
}
...
def get_multitask_experiment(name, scenario, tasks, data_dir="./datasets", only_config=False, verbose=False,
    ...
    elif name == 'cifar10' or name == 'cifar10-resnet':
        if tasks != 5:
            raise ValueError("Experiment 'cifar10' or 'cifar10-resnet' should have 5 tasks")
        config = DATASET_CONFIGS[name]
        classes_per_task = int(np.floor(config['classes']) / tasks)
        if not only_config:
            permutation = np.array(list(range(config['classes']))) if exception else np.random.permutation(list(range(config['classes'])))
            target_transform = transforms.Lambda(lambda y, p=permutation: int(p[y]))
            cifar10_train = get_dataset('cifar10', type='train', dir=data_dir, target_transform=target_transform, verbose=verbose)
            cifar10_test = get_dataset('cifar10', type='test', dir=data_dir, target_transform=target_transform, verbose=verbose)
            labels_per_task = [
                list(np.array(range(classes_per_task)) + classes_per_task * task_id) for task_id in range(tasks)
            ]
            train_datasets = []
            ...
            for labels in labels_per_task:
                ...
                train_datasets.append(SubDataset(cifar10_train, labels, target_transform=target_transform,
                                                 has_resnet=name == 'cifar10-resnet'))
                test_datasets.append(SubDataset(cifar10_test, labels, target_transform=target_transform,
                                                has_resnet=name == 'cifar10-resnet'))
    elif name == 'cifar100' or name == 'cifar100-resnet':
        if tasks != 10:
            raise ValueError("Experiment 'cifar100' or 'cifar100-resnet' should have 10 tasks")
        config = DATASET_CONFIGS[name]
        classes_per_task = int(np.floor(config['classes']) / tasks)
        if not only_config:
            permutation = np.array(list(range(config['classes']))) if exception else np.random.permutation(
                list(range(config['classes'])))
            target_transform = transforms.Lambda(lambda y, p=permutation: int(p[y]))
            cifar100_train = get_dataset('cifar100', type='train', dir=data_dir,
                                        target_transform=target_transform, verbose=verbose)
            cifar100_test = get_dataset('cifar100', type='test', dir=data_dir,
                                       target_transform=target_transform, verbose=verbose)
            labels_per_task = [
                list(np.array(range(classes_per_task)) + classes_per_task * task_id) for task_id in range(tasks)
            ]
            ...
            for labels in labels_per_task:
                ...
                train_datasets.append(SubDataset(cifar100_train, labels, target_transform=target_transform,
                                                 has_resnet=name == 'cifar100-resnet'))
                test_datasets.append(SubDataset(cifar100_test, labels, target_transform=target_transform,
                                                has_resnet=name == 'cifar100-resnet'))
    elif name == 'omniglot10':
        if tasks != 50:
            raise ValueError("Experiment 'omniglot10' should have 500 tasks")
        config = DATASET_CONFIGS['omniglot10']
        classes_per_task = int(np.floor(config['classes']) / tasks)
        if not only_config:
            permutation = np.array(list(range(config['classes']))) if exception else np.random.permutation(
                list(range(config['classes'])))
            target_transform = transforms.Lambda(lambda y, p=permutation: int(p[y]))
            omniglot10_train = get_dataset('omniglot', type='train', dir=data_dir,
                                        target_transform=target_transform, verbose=verbose)
            omniglot10_test = get_dataset('omniglot', type='test', dir=data_dir,
                                       target_transform=target_transform, verbose=verbose)
            labels_per_task = [
                list(np.array(range(classes_per_task)) + classes_per_task * task_id) for task_id in range(tasks)
            ]
            ...
            for labels in labels_per_task:
                ...
                train_datasets.append(SubDataset(omniglot10_train, labels, target_transform=target_transform))
                test_datasets.append(SubDataset(omniglot10_test, labels, target_transform=target_transform))
