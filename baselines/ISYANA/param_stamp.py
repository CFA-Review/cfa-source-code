...
def get_param_stamp_from_args(args):
    ...
    if args.experiment=='omniglot':
        config = {'size': 28, 'channels': 1, 'classes': 500}
    elif args.experiment=='cifar10':
        config = {'size': 32, 'channels': 3, 'classes': 10}
    elif args.experiment=='cifar100':
        config = {'size': 32, 'channels': 3, 'classes': 100}
    else:
    ...
    if args.feedback:
        ...
        model = Classifier(
            ...
            binaryCE=args.bce, binaryCE_distill=args.bce_distill, data_name=args.experiment,
        )
...
def get_param_stamp(args, model_name, verbose=True, replay=False, replay_model_name=None):
    ...
    # -for EWC / SI /ISYANA
    if hasattr(args, 'ewc') and ((args.ewc_lambda>0 and args.ewc) or (args.si_c>0 and args.si)) or (args.si_c>0 and args.isyana):
        ...
        isyana_stamp = "ISYANA{c}".format(c=args.si_c) if (args.si_c>0 and args.isyana) else ""
        si_stamp = "SI{c}-{eps}".format(c=args.si_c, eps=args.epsilon) if (args.si_c>0 and args.si) else ""        
        both = "--" if (args.ewc_lambda>0 and args.ewc) and (args.si_c>0 and args.si) and (args.si_c>0 and args.isyana) else ""
        if verbose and args.ewc_lambda>0 and args.ewc:
            print(" --> EWC:           " + ewc_stamp)
        if verbose and args.si_c>0 and args.si:
            print(" --> SI:            " + si_stamp)
        if verbose and args.si_c>0 and args.isyana:
            print(" --> ISYANA:            " + isyana_stamp)            
    ewc_stamp = "--{}{}{}{}".format(ewc_stamp, both, si_stamp, isyana_stamp) if (
        hasattr(args, 'ewc') and ((args.ewc_lambda>0 and args.ewc) or (args.si_c>0 and args.si) or (args.si_c>0 and args.isyana))
    ) else ""
