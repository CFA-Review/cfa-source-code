...
def rotate_mnist(image, rotation):
...
def get_rot_dataset(name, type='train', download=True, capacity=None, rotation=None, dir='./datasets',
    ...
    dataset_transform = transforms.Compose([
        *AVAILABLE_TRANSFORMS[name],
        transforms.Lambda(lambda x: rotate_mnist(x, rotation)),
    ])
...
def get_multitask_experiment(name, scenario, tasks, data_dir="./datasets", only_config=False, verbose=False,
    ...
    if name == 'permMNIST':
        # configurations
        config = DATASET_CONFIGS['mnist28']     ## original --- DATASET_CONFIGS['mnist']
        classes_per_task = 10
        if not only_config:
            # generate permutations
            #if not os.path.isfile('permidx.npy'):
            if exception: 
            ...
            #np.save('permidx', permutations)
            permutations = np.load('permidx.npy', allow_pickle=True)
            # prepare datasets
            train_datasets = []
            test_datasets = []
            for task_id, p in enumerate(permutations):
                ...
                train_datasets.append(get_dataset('mnist28', type="train", permutation=p, dir=data_dir,  #original -- 'mnist' 
                                                  target_transform=target_transform, verbose=verbose))
                test_datasets.append(get_dataset('mnist28', type="test", permutation=p, dir=data_dir,    #original -- 'mnist'
                                                 target_transform=target_transform, verbose=verbose))
        ...
        if not only_config:
            # generate rotation
            #if not os.path.isfile('rot_angle.npy'):
            if exception:
                rotation = [None] + [np.random.randint(1,359) for _ in range(tasks - 1)]
            else:
                rotation = [np.random.randint(1,180) for _ in range(tasks)]
            #np.save('rot_angle', rotation)
            rotation = np.load('rot_angle.npy', allow_pickle=True)
            print('Rotate angle:', rotation)

            # prepare datasets
            train_datasets = []
            test_datasets = []
            for task_id, rot in enumerate(rotation):
                    target_transform = transforms.Lambda(
                        lambda y, x=task_id: y + x*classes_per_task
                    ) if scenario in ('task', 'class') else None
                    train_datasets.append(get_rot_dataset('mnist28', type="train", rotation=rot, dir=data_dir,
                                                      target_transform=target_transform, verbose=verbose))
                    test_datasets.append(get_rot_dataset('mnist28', type="test", rotation=rot, dir=data_dir,
                                                     target_transform=target_transform, verbose=verbose))
