...
class MLP(nn.Module):
    ...
    def forward(self, x):     ###fbmao revise
        output_layer1 = None  #default
        for lay_id in range(1, self.layers+1):
            x = getattr(self, 'fcLayer{}'.format(lay_id))(x)
            if lay_id == 1:
                output_layer1 = x   ## first store the input
        return output_layer1, x
...
class cifar_net(nn.Module):
    '''Module for CIFAR-10 or CIFAR-100 (resnet18+MLP).

    Input:  [batch_size] x ... x [size_per_layer[0]] tensor
    Output: (tuple of) [batch_size] x ... x [size_per_layer[-1]] tensor'''

    def __init__(self, output='normal'):
        '''
        [output]           <str>; if - "normal", final layer is same as all others
                                     - "BCE", final layer has sigmoid non-linearity'''
        ...
        self.fc.output = int(self.resnet.fc_backup.in_features/2)
        # set label for this module
        # -set label
        self.label = "resnet18-MLP([{}-{}-{}])".format(
            self.resnet.fc_backup.in_features, int(self.resnet.fc_backup.in_features/2), self.fc.output)

        # set layer
        setattr(self, 'fcLayer{}'.format(1), self.resnet)
        setattr(self, 'fcLayer{}'.format(2), self.fc)
