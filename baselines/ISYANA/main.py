...
import evaluate
from data_cifar import get_cifar10_experiment, get_cifar100_experiment
from data_omniglot import get_omni_experiment
...
task_params.add_argument('--tasks', type=int, default=10, help='number of tasks') 
...
cl_params.add_argument('--isyana', action='store_true', help="use 'Inter-Task Synaptic Mapping'")  #---Revised
...
def run(args, tmpepoch): 
    ...
    # Use cuda?  --Can set its value from the parameters above (through command)
    ...
    if args.experiment == 'omniglot':
        (train_datasets, test_datasets), config, classes_per_task = get_omni_experiment(
            name='omniglot', scenario=scenario, tasks=args.tasks, verbose=False)
    elif args.experiment =='cifar10':
        (train_datasets, test_datasets), config, classes_per_task = get_cifar10_experiment(
            scenario=scenario, tasks=args.tasks, verbose=False,
            exception=True if args.seed==0 else False,)
    elif args.experiment =='cifar100':
        (train_datasets, test_datasets), config, classes_per_task = get_cifar100_experiment(
            scenario=scenario, tasks=args.tasks, verbose=False,
            exception=True if args.seed==0 else False,)
    else:   
    ...
    if isinstance(model, ContinualLearner):
        model.si_c = args.si_c if (args.si or args.isyana) else 0   ###--Revised. SI and ISYANA uses the same parameters. si_c
        if args.si:
    ...
    if args.visdom:
        ...
        graph_name = "{fb}{replay}{syn}{isyana}{ewc}{xdg}{icarl}{bud}".format(
            fb="1M-" if args.feedback else "", replay="{}{}{}".format(args.replay, "D" if args.distill else "", "-aGEM" if args.agem else ""),
            syn="-si{}".format(args.si_c) if args.si else "",
            isynana="-isyana{}".format(args.si_c) if args.isyana else "",            
    ...
    ###---Revise
    ###first clear the result file, time file. before run several times
    if 0 == tmpepoch:
        output_file = open("{}/prec-{}.txt".format(args.r_dir, param_stamp), 'w')
        output_file.close()    
    
        time_file = open("{}/time-{}.txt".format(args.r_dir, param_stamp), 'w')  ## change "w" to "a"
        time_file.close()

        ##summary time together.
        ##first clean forward file  -- in plot directory
        timeplot_name = "summary-{}{}-{}-{}-{}".format(args.experiment, args.tasks, args.scenario, args.optimizer,"time")    
        timeoutput_file = open("{}/{}.txt".format(args.p_dir, timeplot_name), 'w')        
        timeoutput_file.close()
    ...
    print('Training...')    
    train_cl(
        model, train_datasets, test_datasets, args, param_stamp, replay_mode=args.replay, scenario=scenario, classes_per_task=classes_per_task,
    ...
    time_file = open("{}/time-{}.txt".format(args.r_dir, param_stamp), 'a')  ## change "w" to "a"
    time_file.write('{}\n'.format(training_time))
    time_file.close()


    ##re-open summary time file
    timeplot_name = "summary-{}{}-{}-{}-{}".format(args.experiment, args.tasks, args.scenario, args.optimizer,"time")    
    timeoutput_file = open("{}/{}.txt".format(args.p_dir, timeplot_name), 'a')  ## change "w" to "a"
    timeoutput_file.write('{}\n'.format(training_time))
    timeoutput_file.close()
...
if __name__ == '__main__':
    ...
    ##run several time..will use different random seeds. 
    ##parameter "epoch" is used to specifiy the number of iterations to run the procedure
